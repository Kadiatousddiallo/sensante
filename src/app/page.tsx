  ...
  return (
    <main className={styles.page}>
      <header className={`${styles.header} flex justify-between items-center bg-white/30 backdrop-blur-md p-6 rounded-2xl shadow-sm mb-12`}>
        <div>
          <h1 className={`text-4xl font-bold text-teal-800 ${styles.title}`}>SénSanté</h1>
          <p className={`text-sm text-gray-600 ${styles.subtitle}`}>Assistant de santé communautaire avec IA</p>
        </div>
        <LoginButton />
      </header>
      ...
      <section className={`${styles.section} mb-16`}>
        ...
        <div className={`${styles.grid} grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3`}>
          <div className={styles.cardWrap}><PatientCard nom="Boubacar Bathily" region="Dakar" age={21} sexe="M" /></div>
          <div className={styles.cardWrap}><PatientCard nom="Fatou Sow" region="Saint-Louis" age={27} sexe="F" /></div>
          <div className={styles.cardWrap}><PatientCard nom="Mamadou Ndiaye" region="Thiès" age={41} sexe="M" /></div>
          <div className={styles.cardWrap}><PatientCard nom="Amina Diallo" region="Ziguinchor" age={34} sexe="F" /></div>
        </div>
      </section>
      ...
      <footer className="mt-auto pt-8 border-t border-teal-100 text-center">
        ...
        <p className="mt-2 text-xs text-teal-600/50 font-medium">SénSanté Unified Platform v1.0</p>
      </footer>
    </main>
  );
}
